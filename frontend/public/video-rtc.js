...
export class VideoRTC extends HTMLElement {
        ...
        pc.addEventListener('connectionstatechange', () => {
            if (pc.connectionState === 'connected') {
                const tracks = pc.getTransceivers()
                    .filter(tr => tr.currentDirection === 'recvonly' || tr.currentDirection === 'sendrecv')
    ...
    async createOffer(pc) {
        const hasMic = this.media.indexOf('microphone') >= 0;
        const hasAudio = this.media.indexOf('audio') >= 0;
        const hasVideo = this.media.indexOf('video') >= 0;
        ...
        if (hasMic) {
            try {
                ...
                console.warn('Microphone permission / capture error:', e);
            }
        }
